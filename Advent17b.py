...
def main():
    ...
    for i in range(len(initialState)):
        for j in range(len(initialState[0])):
            if initialState[i][j] == '#':
                activeCubes[getPointName((j,i,0,0))] = (j,i,0,0)
            
    #create 80 neighbors tuples
    neighbors = []
    for i in range(1,81):
        xyzw = find_ternary(i).zfill(4)
        if xyzw == '1111':
            neighbors.append((-1,-1,-1, -1))
        else:
            neighbors.append((int(xyzw[0])-1,
                              int(xyzw[1])-1,
                              int(xyzw[2])-1,
                              int(xyzw[3])-1))
    ...
    for i in range(6):
        ...
        w_range = [w for w in range(w_dim[0], w_dim[1])]
    
        cube = [(x,y,z,w) for x in x_range for y in y_range for z in z_range
                for w in w_range]
        addActive = []
        removeActive = []
        for pt in cube:
